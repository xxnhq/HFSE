...
def eval_dnsmos(testset_dir):
    models = glob.glob(os.path.join(testset_dir, "*"))
    audio_clips_list = []
    p808_model_path = '/data2/urgent2024_challenge/DNSMOS/DNSMOS/model_v8.onnx'
    primary_model_path = '/data2/urgent2024_challenge/DNSMOS/DNSMOS/sig_bak_ovr.onnx'
    ...
    return round(df['SIG'].mean(), 3), round(df['BAK'].mean(), 3), round(df['OVRL'].mean(), 3)
